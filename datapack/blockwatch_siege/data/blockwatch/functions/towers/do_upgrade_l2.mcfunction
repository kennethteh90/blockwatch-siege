...
execute unless score @a[tag=bw.upgrading,limit=1] bw.gold >= #upg.cost.1 bw.cfg run tellraw @a[tag=bw.upgrading,limit=1] [{"text":"[Siege] ","color":"gold"},{"text":"Not enough gold — upgrade costs 30g.","color":"red"}]
execute unless score @a[tag=bw.upgrading,limit=1] bw.gold >= #upg.cost.1 bw.cfg run return 0

scoreboard players operation @a[tag=bw.upgrading,limit=1] bw.gold -= #upg.cost.1 bw.cfg
...
tellraw @a[tag=bw.upgrading,limit=1] [{"text":"[Siege] ","color":"gold"},{"text":"Upgraded to Lv 2! (-30g)  Faster attack rate.","color":"green"}]
