...
execute unless score #gold bw.gold >= #upg.cost.2 bw.cfg run tellraw @a[tag=bw.upgrading,limit=1] [{"text":"[Siege] ","color":"gold"},{"text":"Team bank is too low — upgrade costs 50g.","color":"red"}]
execute unless score #gold bw.gold >= #upg.cost.2 bw.cfg run return 0

scoreboard players operation #gold bw.gold -= #upg.cost.2 bw.cfg
...
tellraw @a[tag=bw.upgrading,limit=1] [{"text":"[Siege] ","color":"gold"},{"text":"Upgraded to Lv 3! (-50g team gold)  MAX — bonus damage","color":"gold"}]
